...
//return value{{{
	return c*( N1*( frac*U1S_t1_val + (1-frac)*U1S_t2_val ) )/
			( N1*( frac*U1S_t1_val + (1-frac)*U1S_t2_val )
			...
			*( 1 - ( N1*( frac*U1S_t1_val + (1-frac)*U1S_t2_val ) )/
			( N1*( frac*U1S_t1_val + (1-frac)*U1S_t2_val )
			+ N4*( TMath::Exp(-x[0]/Bkgp0)*(TMath::Erf((x[0]-Bkgmean)/(TMath::Sqrt(2)*Bkgsigma))+1)/2.) ) );
//}}}
...
//return value{{{
	return c*( N1*( frac*U1S_t1_val + (1-frac)*U1S_t2_val )
			+ N2*( frac*U2S_t1_val + (1-frac)*U2S_t2_val ) )/
			( N1*( frac*U1S_t1_val + (1-frac)*U1S_t2_val )
			+ N2*( frac*U2S_t1_val + (1-frac)*U2S_t2_val )
			+ N4*( TMath::Exp(-x[0]/Bkgp0)*(TMath::Erf((x[0]-Bkgmean)/(TMath::Sqrt(2)*Bkgsigma))+1)/2. ) )
			+ ( d*TMath::Exp(-x[0]/ep)*(TMath::Erf((x[0]-em)/(TMath::Sqrt(2)*es))+1)/2. )
...
//return value{{{
	return c*( N1*( frac*U1S_t1_val + (1-frac)*U1S_t2_val )
			+ N2*( frac*U2S_t1_val + (1-frac)*U2S_t2_val )
			+ N3*( frac*U3S_t1_val + (1-frac)*U3S_t2_val ) )/
			( N1*( frac*U1S_t1_val + (1-frac)*U1S_t2_val )
			+ N2*( frac*U2S_t1_val + (1-frac)*U2S_t2_val )
			+ N3*( frac*U3S_t1_val + (1-frac)*U3S_t2_val )
			+ N4*( TMath::Exp(-x[0]/Bkgp0)*(TMath::Erf((x[0]-Bkgmean)/(TMath::Sqrt(2)*Bkgsigma))+1)/2. ) )
...
//totalvn pol1 bkg Upsilon 1S{{{
Double_t Totalvnpol1U1S(Double_t* x, Double_t* par)
{
	Double_t N1 = par[0];
	Double_t N2 = par[1];
	Double_t N3 = par[2];
...
//totalvn pol1 bkg Upsilon 12S{{{
Double_t Totalvnpol1U12S(Double_t* x, Double_t* par)
{
	Double_t N1 = par[0];
	Double_t N2 = par[1];
	Double_t N3 = par[2];
...
void ObservedV2fit(const Int_t multMin = 0, const Int_t multMax = 300, const Double_t rapMin = -2.4, const Double_t rapMax = 2.4, const Double_t TrkptMin = 0, const Double_t TrkptMax = 1, const Int_t maxF = 2, const Int_t bkgN = 0, const Int_t NU = 1, const Int_t detacut = 1, const Bool_t isfine = true, TString version = "v13", TString MupT = "4")
{
	SetStyle();
...
//Make directory{{{
	...
	TString fDIR = mainDIR + "/V2Dist/V2File";
	TString sDIR = mainDIR + "/V2Dist/SigV2";
...
//}}}
	...
	TFile* fout;
	if(maxF == 2) fout = new TFile(Form("V2Dist/V2File/Observed_fit_Mult_%d-%d_rap_%d-%d_Trkpt_%d-%d_%s_%s_%s_%s_%s_MupT%s.root", (int)multMin, (int)multMax, (int)(10*rapMin), (int)(10*rapMax), (int)TrkptMin, (int)TrkptMax, bkgF.Data(), Ups.Data(), DEC.Data(), Fine.Data(), version.Data(), MupT.Data()), "RECREATE");
	else if(maxF == 3) fout = new TFile(Form("V2Dist/V2File/Observed_fit_Mult_%d-%d_rap_%d-%d_Trkpt_%d-%d_%s_%s_%s_%s_%s_MupT%s_tra.root", (int)multMin, (int)multMax, (int)(10*rapMin), (int)(10*rapMax), (int)TrkptMin, (int)TrkptMax, bkgF.Data(), Ups.Data(), DEC.Data(), Fine.Data(), version.Data(), MupT.Data()), "RECREATE");
	else
	{
...
//Define canvas for yield and vn dist{{{
	...
	{
		c1[ipt] = new TCanvas(Form("c1_%d", ipt), "", 0, 0, 600, 600);
...
//Get fitting parameter{{{
		...
		ifstream in1;
		in1.open(Form("../SkimmedFiles/Parameter/Result_parameters_mult_%d-%d_pt_%d-%d_rap_%d-%d_%s_MupT%s.txt", (int)multMin, (int)multMax, (int)(10*ptBinsArr[ipt]), (int)(10*ptBinsArr[ipt+1]), (int)(10*rapMin), (int)(10*rapMax), version.Data(), MupT.Data()));
		if(in1.is_open())
		{
		...
		ifstream in2;
		in2.open(Form("Parameter/Obv2_bkg_%s_par_mult_%d-%d_pt_%d-%d_rap_%d-%d_Trkpt_%d-%d_%s_MupT%s.txt", bkgF.Data(), (int)multMin, (int)multMax, (int)(10*ptBinsArr[ipt]), (int)(10*ptBinsArr[ipt+1]), (int)(10*rapMin), (int)(10*rapMax), (int)TrkptMin, (int)TrkptMax, version.Data(), MupT.Data()));
		if(in2.is_open())
		{
...
//Get vn distribution{{{
		TFile* fvn;
		if(maxF == 2) fvn = new TFile(Form("../ExtractV2/ProjDist/DistFiles/v2_dist_Mult_%d-%d_pt_%d-%d_rap_%d-%d_Trkpt_%d-%d_%s_MupT%s.root", (int)multMin, (int)multMax, (int)(ptBinsArr[ipt]), (int)(ptBinsArr[ipt+1]), (int)(10*rapMin), (int)(10*rapMax), (int)TrkptMin, (int)TrkptMax, version.Data(), MupT.Data()), "READ");
		else if(maxF == 3) fvn = new TFile(Form("../ExtractV2/ProjDist/DistFiles/v2_dist_Mult_%d-%d_pt_%d-%d_rap_%d-%d_Trkpt_%d-%d_%s_MupT%s_tra.root", (int)multMin, (int)multMax, (int)(ptBinsArr[ipt]), (int)(ptBinsArr[ipt+1]), (int)(10*rapMin), (int)(10*rapMax), (int)TrkptMin, (int)TrkptMax, version.Data(), MupT.Data()), "READ");
		else
		{
			cout << "No such Fourier value in this analysis" << endl;
			return;
...
//define function for fitting{{{
		...
		if(bkgN == 0)
		{
			if(NU == 1) fitvn = new TF1(Form("fitvn_%d", ipt), TotalvnexpU1S, 8, 14, 18);
			else if(NU == 2) fitvn = new TF1(Form("fitvn_%d", ipt), TotalvnexpU12S, 8, 14, 18);
			else if(NU == 3) fitvn = new TF1(Form("fitvn_%d", ipt), TotalvnexpU123S, 8, 14, 18);
			else
			{
		...
		{
			if(NU == 1) fitvn = new TF1(Form("fitvn_%d", ipt), Totalvnpol2U1S, 8, 14, 17);
			else if(NU == 2) fitvn = new TF1(Form("fitvn_%d", ipt), Totalvnpol2U12S, 8, 14, 17);
			else if(NU == 3) fitvn = new TF1(Form("fitvn_%d", ipt), Totalvnpol2U123S, 8, 14, 17);
			else
			{
				cout << "No such Upsilon generation in this analysis" << endl;
				return;
		...
		{
			if(NU == 1) fitvn = new TF1(Form("fitvn_%d", ipt), Totalvnpol1U1S, 8, 14, 16);
			else if(NU == 2) fitvn = new TF1(Form("fitvn_%d", ipt), Totalvnpol1U12S, 8, 14, 16);
			else if(NU == 3) fitvn = new TF1(Form("fitvn_%d", ipt), Totalvnpol1U123S, 8, 14, 16);
			else
			{
				cout << "No such Upsilon generation in this analysis" << endl;
				return;
...
//parameter setting{{{
		...
		c1[ipt]->cd();
		hist->Draw();
		fitvn->SetParameter(0, var21/10.);
		fitvn->SetParameter(1, var22/10.);
		fitvn->SetParameter(2, var23/10.);
		fitvn->SetParameter(3, var24/10.);
		fitvn->FixParameter(4, var13);
		fitvn->FixParameter(5, var14);
		fitvn->FixParameter(6, var16);
		fitvn->FixParameter(7, var17);
		fitvn->FixParameter(8, var3/var2);
		fitvn->FixParameter(9, var15);
		fitvn->FixParameter(10, var18);
		fitvn->FixParameter(11, var19);
		fitvn->FixParameter(12, var20);
		fitvn->SetParameter(13, 0.01);
		fitvn->SetParameter(14, bpar1);
		fitvn->SetParameter(15, bpar2);
		fitvn->SetParameter(16, bpar3);
		fitvn->SetParameter(17, bpar4);
//}}}
...
//vn fitting result{{{
		...
		if(bkgN == 0)
		{
			...
			fvn_bkg->FixParameter(0, fitvn->GetParameter(14));
			fvn_bkg->FixParameter(1, fitvn->GetParameter(15));
			fvn_bkg->FixParameter(2, fitvn->GetParameter(16));
			fvn_bkg->FixParameter(3, fitvn->GetParameter(17));
		}
		else if(bkgN == 1)
		{
...
//store individual function{{{
		fout->cd();
		fvntot[ipt] = (TF1*) fitvn->Clone();
		fvntot[ipt]->SetName(Form("vnfit_pt%d", ipt));
		fvntot[ipt]->Write();
//}}}

		v2[ipt] = fitvn->GetParameter(13);
		v2e[ipt] = fitvn->GetParError(13);
		//v2_bkg[ipt] = fitvn->GetParameter(14) + fitvn->GetParameter(14) * U1S_mass;
...
//get Chi2{{{
		...
		for(Int_t ibin = 0; ibin < gvn->GetN(); ibin++)
		{
			...
			pullv2[ibin] = (v2y[ibin] - fitvn->Eval(xv2[ibin]))/gvn->GetErrorY(ibin);
		...
		lt1->DrawLatex(0.42, 0.85, Form("p_{T}^{#mu} < %.1f GeV/c", MupTCut));
		lt1->DrawLatex(0.42, 0.8, Form("%d < p_{T}^{#varUpsilon} < %d GeV/c", (int)ptBinsArr[ipt], (int)ptBinsArr[ipt+1]));
		if((int) TrkptMin == 0) lt1->DrawLatex(0.42,0.75, Form("0.3 < p_{T}^{trk} < %d GeV/c", (int) TrkptMax));
		else lt1->DrawLatex(0.42,0.75, Form("%d < p_{T}^{assoc} < %d GeV/c", (int) TrkptMin, (int) TrkptMax));
		lt1->DrawLatex(0.42,0.7, "|#Delta#eta^{trk}| > 1.0");
...
//}}}
	...
	for(Int_t ipt = 0; ipt < pt_narr-1; ipt++)
	{
		if(maxF == 2) c1[ipt]->SaveAs(Form("V2Dist/SigV2/Observed_fit_Mult_%d-%d_pt_%d-%d_rap_%d-%d_Trkpt_%d-%d_%s_%s_%s_%s_%s_MupT%s.pdf", (int)multMin, (int)multMax, (int)(ptBinsArr[ipt]*10), (int)(ptBinsArr[ipt+1]*10), (int)(10*rapMin), (int)(10*rapMax), (int)TrkptMin, (int)TrkptMax, bkgF.Data(), Ups.Data(), DEC.Data(), Fine.Data(), version.Data(), MupT.Data()));
		else if(maxF == 3) c1[ipt]->SaveAs(Form("V2Dist/SigV2/Observed_fit_Mult_%d-%d_pt_%d-%d_rap_%d-%d_Trkpt_%d-%d_%s_%s_%s_%s_%s_MupT%s_tra.pdf", (int)multMin, (int)multMax, (int)(ptBinsArr[ipt]*10), (int)(ptBinsArr[ipt+1]*10), (int)(10*rapMin), (int)(10*rapMax), (int)TrkptMin, (int)TrkptMax, bkgF.Data(), Ups.Data(), DEC.Data(), Fine.Data(), version.Data(), MupT.Data()));
		else
		{
			cout << "No such Fourier value in this analysis" << endl;
			return;
