...
void FreePar(const Int_t multMin = 0, const Int_t multMax = 300, const Double_t rapMin = -2.4, const Double_t rapMax = 2.4, const Double_t TrkptMin = 0, const Double_t TrkptMax = 1, const Bool_t isfine = true, const TString version = "v13", const TString MupT = "4", const Int_t FreeParN = 0)
{
	SetStyle();
...
//Get vn distribution{{{
		...
		else gvn = (TGraphErrors*) fvn->Get(Form("gv2_Away1_coarse"));
		c1[ipt]->cd(2);
...
//define function for simultaneous fitting{{{
		...
		TF1* fvn_simul = new TF1(Form("fvn_simul_%d", ipt), Totalvnpol2U123S, RangeMin, RangeMax, 19);
//}}}
...
//combined function condition{{{
		...
		if(FreeParN == 5) fitter.Config().ParSettings(9).SetLimits(0., 1.);
		if(FreeParN == 6) fitter.Config().ParSettings(10).SetLimits(0., 10.);
		if(FreeParN == 7) fitter.Config().ParSettings(11).SetLimits(1., 100.);
		if(FreeParN == 8) fitter.Config().ParSettings(12).SetLimits(1., 100.);
		if(FreeParN != 0) fitter.Config().ParSettings(4).Fix();
		if(FreeParN != 1) fitter.Config().ParSettings(5).Fix();
		if(FreeParN != 2) fitter.Config().ParSettings(6).Fix();
		if(FreeParN != 3) fitter.Config().ParSettings(7).Fix();
		if(FreeParN != 4) fitter.Config().ParSettings(8).Fix();
		if(FreeParN != 5) fitter.Config().ParSettings(9).Fix();
		if(FreeParN != 6) fitter.Config().ParSettings(10).Fix();
		if(FreeParN != 7) fitter.Config().ParSettings(11).Fix();
		if(FreeParN != 8) fitter.Config().ParSettings(12).Fix();
...
//vn fitting result{{{
		...
		TF1* fvn_bkg = new TF1(Form("fvn_bkg_%d", ipt), pol2bkg, RangeMin, RangeMax, 3);
