...
//}}}
	...
	return N1*( frac*U1S_t1_val + (1-frac)*U1S_t2_val )
...
void SimultaneousFit(const Int_t multMin = 0, const Int_t multMax = 300, const Double_t rapMin = -2.4, const Double_t rapMax = 2.4, const Double_t TrkptMin = 0, const Double_t TrkptMax = 1, const Int_t maxF = 2, const Int_t bkgN = 0, const Int_t NU = 1, const Int_t detacut = 1, const Bool_t isfine = true, TString version = "v13", TString MupT = "4")
{
	SetStyle();
...
//}}}
	...
	TFile* fout;
	if(maxF == 2) fout = new TFile(Form("V2Dist/V2File/Combine_fit_Mult_%d-%d_rap_%d-%d_Trkpt_%d-%d_%s_%s_%s_%s_%s_MupT%s.root", (int)multMin, (int)multMax, (int)(10*rapMin), (int)(10*rapMax), (int)TrkptMin, (int)TrkptMax, bkgF.Data(), Ups.Data(), DEC.Data(), Fine.Data(), version.Data(), MupT.Data()), "RECREATE");
	else if(maxF == 3) fout = new TFile(Form("V2Dist/V2File/Combine_fit_Mult_%d-%d_rap_%d-%d_Trkpt_%d-%d_%s_%s_%s_%s_%s_MupT%s_tra.root", (int)multMin, (int)multMax, (int)(10*rapMin), (int)(10*rapMax), (int)TrkptMin, (int)TrkptMax, bkgF.Data(), Ups.Data(), DEC.Data(), Fine.Data(), version.Data(), MupT.Data()), "RECREATE");
	else
	{
...
//Get yield distribution{{{
		TFile* fyield = new TFile(Form("../SkimmedFiles/Yield/Yield_Mult_%d-%d_pt_%d-%d_rap_%d-%d_%s_MupT%s.root", (int)multMin, (int)multMax, (int)(ptBinsArr[ipt]), (int)(ptBinsArr[ipt+1]), (int)(10*rapMin), (int)(10*rapMax), version.Data(), MupT.Data() ), "READ");
...
//Get fitting parameter{{{
		...
		ifstream in2;
		in2.open(Form("Parameter/v2_bkg_%s_par_mult_%d-%d_pt_%d-%d_rap_%d-%d_Trkpt_%d-%d_%s_MupT%s.txt", bkgF.Data(), (int)multMin, (int)multMax, (int)(10*ptBinsArr[ipt]), (int)(10*ptBinsArr[ipt+1]), (int)(10*rapMin), (int)(10*rapMax), (int)TrkptMin, (int)TrkptMax, version.Data(), MupT.Data()));
		if(in2.is_open())
		{
...
//define function for simultaneous fitting{{{
		...
		{
			if(NU == 1) fvn_simul = new TF1(Form("fvn_simul_%d", ipt), Totalvnpol1U1S, 8, 14, 16);
			else if(NU == 2) fvn_simul = new TF1(Form("fvn_simul_%d", ipt), Totalvnpol1U12S, 8, 14, 16);
			else if(NU == 3) fvn_simul = new TF1(Form("fvn_simul_%d", ipt), Totalvnpol1U123S, 8, 14, 16);
			else
			{
				cout << "No such Upsilon generation in this analysis" << endl;
				return;
...
//vn fitting result{{{
		...
		{
			fvn_bkg = new TF1(Form("fvn_bkg_%d", ipt), pol1bkg, 8, 14, 2);
			fvn_bkg->FixParameter(0, fvn_simul->GetParameter(14));
			fvn_bkg->FixParameter(1, fvn_simul->GetParameter(15));
		}
		else
		...
		FILE* ftxt;
		ftxt = fopen(Form("Parameter/Obv2_bkg_%s_par_mult_%d-%d_pt_%d-%d_rap_%d-%d_Trkpt_%d-%d_%s_MupT%s.txt", bkgF.Data(), (int)multMin, (int)multMax, (int)(ptBinsArr[ipt]*10), (int)(ptBinsArr[ipt+1]*10), (int)(rapMin*10), (int)(rapMax*10), (int)TrkptMin, (int)TrkptMax, version.Data(), MupT.Data()), "w");
		if(ftxt != NULL)
		{
			fprintf(ftxt, "%f   %f   %f   %f \n", fvn_simul->GetParameter(14), fvn_simul->GetParameter(15), fvn_simul->GetParameter(16), fvn_simul->GetParameter(17));
		}
//}}}
...
//get Chi2{{{
		...
		lt1->DrawLatex(0.42, 0.65, Form("Chi2/ndf = %.f/%d", Chi2Yield, NdfYield));
...
//}}}
	...
	for(Int_t ipt = 0; ipt < pt_narr-1; ipt++)
	{
		if(maxF == 2) c1[ipt]->SaveAs(Form("V2Dist/SigV2/Combine_fit_Mult_%d-%d_pt_%d-%d_rap_%d-%d_Trkpt_%d-%d_%s_%s_%s_%s_%s_MupT%s.pdf", (int)multMin, (int)multMax, (int)(ptBinsArr[ipt]*10), (int)(ptBinsArr[ipt+1]*10), (int)(10*rapMin), (int)(10*rapMax), (int)TrkptMin, (int)TrkptMax, bkgF.Data(), Ups.Data(), DEC.Data(), Fine.Data(), version.Data(), MupT.Data()));
		else if(maxF == 3) c1[ipt]->SaveAs(Form("V2Dist/SigV2/Combine_fit_Mult_%d-%d_pt_%d-%d_rap_%d-%d_Trkpt_%d-%d_%s_%s_%s_%s_%s_MupT%s_tra.pdf", (int)multMin, (int)multMax, (int)(ptBinsArr[ipt]*10), (int)(ptBinsArr[ipt+1]*10), (int)(10*rapMin), (int)(10*rapMax), (int)TrkptMin, (int)TrkptMax, bkgF.Data(), Ups.Data(), DEC.Data(), Fine.Data(), version.Data(), MupT.Data()));
		else
		{
			cout << "No such Fourier value in this analysis" << endl;
			return;
