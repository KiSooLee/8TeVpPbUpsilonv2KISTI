...
//calculate v2 value{{{
	...
	Double_t v2yratio_pl[4];
	Double_t Ev2yratio_pl[4];
	Double_t v2yratio_mi[4];
	Double_t Ev2yratio_mi[4];
	...
	for(Int_t ibin = 0; ibin < gref->GetN(); ibin++)
	{
		...
		gsys_pl->SetPoint(ibin, v2xsys_pl[ibin]+0.5, v2ysys_pl[ibin]);
		gsys_pl->SetPointError(ibin, Exsys_pl[ibin], Eysys_pl[ibin]);
		gsys_mi->SetPoint(ibin, v2xsys_mi[ibin]-0.5, v2ysys_mi[ibin]);
		...
		v2yratio_pl[ibin] = v2ysys_pl[ibin]/v2yref[ibin];
		v2yratio_mi[ibin] = v2ysys_mi[ibin]/v2yref[ibin];
		Ev2yratio_pl[ibin] = TMath::Sqrt(Eyref[ibin]*Eyref[ibin]+Eysys_pl[ibin]*Eysys_pl[ibin]);
		Ev2yratio_mi[ibin] = TMath::Sqrt(Eyref[ibin]*Eyref[ibin]+Eysys_mi[ibin]*Eysys_mi[ibin]);
...
//cout << rate[ibin] << endl;
	...
	lg1->AddEntry(gsys_pl, "Up", "pl");
	lg1->AddEntry(gsys_mi, "Down", "pl");
	lg1->Draw();
	...
	c3->cd();
	hist3->Draw();
	TGraphErrors* gratio_pl = new TGraphErrors(gref->GetN(), v2xref, v2ydiff_pl, Exref, Ev2ydiff_pl);
	gratio_pl->SetName("gv2_ratio_pl");
	gratio_pl->SetMarkerStyle(20);
	gratio_pl->SetMarkerColor(kRed+1);
	gratio_pl->SetMarkerSize(1.4);
	gratio_pl->SetLineColor(kRed+1);
	gratio_pl->Draw("samepe");
	TGraphErrors* gratio_mi = new TGraphErrors(gref->GetN(), v2xref, v2ydiff_mi, Exref, Ev2ydiff_mi);
	gratio_mi->SetName("gv2_ratio_mi");
	gratio_mi->SetMarkerStyle(20);
	gratio_mi->SetMarkerColor(kBlue+1);
	gratio_mi->SetMarkerSize(1.4);
	gratio_mi->SetLineColor(kBlue+1);
	gratio_mi->Draw("samepe");
	SetLine(1, 0, 1, 30, 1, 0, 3);
	TLegend* lg3 = new TLegend(0.6, 0.65, 0.9, 0.85);
	FormLegend(lg3, 0.04);
	lg3->AddEntry(gratio_pl, "Up", "pl");
	lg3->AddEntry(gratio_mi, "Down", "pl");
	lg3->Draw();
