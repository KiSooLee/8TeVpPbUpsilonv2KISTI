...
void RatioNProjection(const Bool_t isMC = false, const Bool_t isTrk, const Int_t multMin = 0, const Int_t multMax = 300, const Double_t ptMin = 0, const Double_t ptMax = 30, const Double_t rapMin = -2.4, const Double_t rapMax = 2.4, const Double_t TrkptMin = 0, const Double_t TrkptMax = 1, const TString version = "v1", const TString MupT = "4")
{ 
	SetStyle();
	gStyle->SetOptFit(0);
...
//fine{{{
	...
	TH1D *hSameRecoDeltaPhi_fine[3][2];
	TH1D *hSameGenDeltaPhi_fine[3][2];
	TH1D *hMixRecoDeltaPhi_fine[3][2];
	TH1D *hMixGenDeltaPhi_fine[3][2];
//}}}
...
//coarse{{{
	...
	TH1D *hSameRecoDeltaPhi_coarse[3][2];
	TH1D *hSameGenDeltaPhi_coarse[3][2];
	TH1D *hMixecoDeltaPhi_coarse[3][2];
	TH1D *hMixenDeltaPhi_coarse[3][2];
//}}}
...
//Store dphi projection{{{
			...
			if(isMC)
			{
				...
				hRatioGenDeltaPhi_fine[iaway]->Write();
				hRatioGenDeltaPhi_coarse[iaway]->Write();
			}
			...
			hRatioRecoDeltaPhi_fine[iaway]->Write();
			hRatioRecoDeltaPhi_coarse[iaway]->Write();
//}}}
		}
		fout->Close();
	}
