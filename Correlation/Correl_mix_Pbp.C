...
//Reco{{{
			...
			{
//cout << "itrg: " <<  itrg << endl;
//cout << "ntrig again: " << Ntrg_Reco1 << endl;
...
//correlation{{{
				for(Int_t irand = 0; irand < 10; irand++)
				{
//cout << "ntrig again1: " << Ntrg_Reco1 << endl;
					Int_t rNum = gRandom->Integer(Nevt2);
//cout << "ntrig again2: " << Ntrg_Reco1 << endl;
					tin2->GetEntry(rNum);
//cout << "ntrig again3: " << Ntrg_Reco1 << endl;
					if(zVtx2 < 0 || TMath::Abs(zVtx1 - zVtx2) > 20.)
					{
						irand--;
						continue;
...
//correlation{{{
				...
				{
					Int_t rNum = gRandom->Integer(Nevt1);
//cout << "ntrig again1: " << Ntrg_Reco2 << endl;
					tin1->GetEntry(rNum);
//cout << "ntrig again2: " << Ntrg_Reco2 << endl;
					if(zVtx1 < 0 || TMath::Abs(zVtx1 - zVtx2) > 20.)
					{
						irand--;
						continue;
					}
//cout << "ntrig again3: " << Ntrg_Reco2 << endl;
					if(Nass_Reco1 <= 0) continue;
					for(Int_t itrk = 0; itrk < Nass_Reco1; itrk++)
					{
//cout << "ntrig again4: " << Ntrg_Reco2 << ", " << Nass_Reco1 << endl;
						vec_ass_Reco1 = (TLorentzVector*) Vec_ass_Reco1->At(itrk);
//cout << "ntrig again5: " << Ntrg_Reco2 << ", " << Nass_Reco1 << endl;
...
//store{{{
	TFile* fout = new TFile(Form("%d-%d_%d-%d_%d-%d_%d-%d_%s_MupT%s/deta-dphi_Pbp_distribution_mix_%s_%d.root", multMin, multMax, (int)ptMin, (int)ptMax, (int)(rapMin*10), (int)(rapMax*10), (int)TrkptMin, (int)TrkptMax, version.Data(), MupT.Data(), MorD.Data(), imass), "RECREATE");
	fout->cd();
	hReco1_1->Write();
	hReco1_2->Write();
	hReco2_1->Write();
