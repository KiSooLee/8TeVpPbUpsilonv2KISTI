...
void Correl_mix_pPb(const bool isMC = false, const Int_t multMin = 0, const Int_t multMax = 300, const Double_t ptMin = 0, const Double_t ptMax = 30, const Double_t rapMin = -2.4, const Double_t rapMax = 2.4, const Double_t TrkptMin = 0, const Double_t TrkptMax = 1, const TString version = "v1", const TString MupT = "4", const Int_t imass = 0)
{
	SetStyle();
...
//Get files{{{
	...
	{
		fname1 = Form("root://cms-xrdr.private.lo:2094///xrd/store/user/kilee/pPb_8TeV_OniaTrkTree/resultpPb1/%d-%d_%d-%d_%d-%d_%d-%d_MupT%s/Sort_OniaTree_pPb1_PADoubleMuon_%s_%d.root", multMin, multMax, (int)ptMin, (int)ptMax, (int)(rapMin*10), (int)(rapMax*10), (int)TrkptMin, (int)TrkptMax, MupT.Data(), MorD.Data(), ibin);
		fname2 = Form("root://cms-xrdr.private.lo:2094///xrd/store/user/kilee/pPb_8TeV_OniaTrkTree/resultpPb2/%d-%d_%d-%d_%d-%d_%d-%d_MupT%s/Sort_OniaTree_pPb2_PADoubleMuon_%s_%d.root", multMin, multMax, (int)ptMin, (int)ptMax, (int)(rapMin*10), (int)(rapMax*10), (int)TrkptMin, (int)TrkptMax, MupT.Data(), MorD.Data(), ibin);
...
//}}}
	...
	if(Nevt1 > 0 && Nevt2 > 0)
	{
		...
		{
			if(ievt%1000 == 0) cout << "1st Events: " << ievt << " / " << Nevt1 << " [" << Form("%.1f", ((double)ievt/(double)Nevt1)*100) << " %]" << endl;
			tin1->GetEntry(ievt);

			if(isMC)
			{
//Gen{{{
				if(Ntrg_Gen1 <= 0 || zVtx1 < 0) continue;
...
//Reco{{{
			if(Ntrg_Reco1 <= 0 || zVtx1 < 0) continue;
...
//fill hist{{{
					hReco1_1->Fill(deta, dphi, 1/(double)(10*Ntrg_Reco1));
					hReco1_2->Fill(deta, dphi, 1/(double)(10*Ntrg_Reco1));
					if(fabs(deta) > 2.0)
					{
						hReco2_1->Fill(deta, dphi, 1/(double)(10*Ntrg_Reco1));
						hReco2_2->Fill(deta, dphi, 1/(double)(10*Ntrg_Reco1));
					}
					if(fabs(deta) > 1.5)
					{
						hReco3_1->Fill(deta, dphi, 1/(double)(10*Ntrg_Reco1));
						hReco3_2->Fill(deta, dphi, 1/(double)(10*Ntrg_Reco1));
					}
					if(fabs(deta) > 1.0)
					{
...
//}}}
		...
		{
			if(ievt%1000 == 0) cout << "2nd Events: " << ievt << " / " << Nevt2 << " [" << Form("%.1f", ((double)ievt/(double)Nevt2)*100) << " %]" << endl;
			tin2->GetEntry(ievt);

			if(isMC)
			{
//Gen{{{
				if(Ntrg_Gen2 <= 0 || zVtx2 < 0) continue;
...
//Reco{{{
			if(Ntrg_Reco2 <= 0 || zVtx2 < 0) continue;
...
//fill hist{{{
						hReco1_1->Fill(deta, dphi, 1/(double)(10*Ntrg_Reco2));
						hReco1_2->Fill(deta, dphi, 1/(double)(10*Ntrg_Reco2));
						if(fabs(deta) > 2.0)
						{
							hReco2_1->Fill(deta, dphi, 1/(double)(10*Ntrg_Reco2));
							hReco2_2->Fill(deta, dphi, 1/(double)(10*Ntrg_Reco2));
						}
						if(fabs(deta) > 1.5)
						{
							hReco3_1->Fill(deta, dphi, 1/(double)(10*Ntrg_Reco2));
							hReco3_2->Fill(deta, dphi, 1/(double)(10*Ntrg_Reco2));
						}
						if(fabs(deta) > 1.0)
						{
...
//store{{{
	TFile* fout = new TFile(Form("%d-%d_%d-%d_%d-%d_%d-%d_%s_MupT%s/deta-dphi_pPb_distribution_mix_%s_%d.root", multMin, multMax, (int)ptMin, (int)ptMax, (int)(rapMin*10), (int)(rapMax*10), (int)TrkptMin, (int)TrkptMax, version.Data(), MupT.Data(), MorD.Data(), imass), "RECREATE");
	fout->cd();
	hReco1_1->Write();
	hReco1_2->Write();
	hReco2_1->Write();
