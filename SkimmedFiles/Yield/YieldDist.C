...
void YieldDist(const TString version = "v1", const Bool_t Weight = true, const Bool_t isAccRW = true, const Bool_t isEffRW = true, const Bool_t isTnP = true, const Bool_t SigSys = false, const Bool_t BkgSys = false, const TString MupT = "4")
{
	SetStyle();
...
//}}}
	...
	for(Int_t imult = 0; imult < mult_narr-1; imult++)
	{
		for(Int_t ipt = 0; ipt < pt_narr-1; ipt++)
		{
			...
			{
				TFile* fin;
				if(Weight) fin  = new TFile(Form("Yield_Mult_%d-%d_pt_%d-%d_rap_%d-%d_Data_%s_Acc%o_Eff%o_TnP%o_SigSys%o_BkgSys%o_MupT%s.root", multBinsArr[imult], multBinsArr[imult+1], (int)ptBinsArr[ipt], (int)ptBinsArr[ipt+1], (int)(10*rapBinsArr[irap]), (int)(10*rapBinsArr[irap+1]), version.Data(), isAccRW, isEffRW, isTnP, SigSys, BkgSys, MupT.Data()), "READ");
				else fin  = new TFile(Form("Yield_Mult_%d-%d_pt_%d-%d_rap_%d-%d_Data_%s_noWeight_MupT%s.root", multBinsArr[imult], multBinsArr[imult+1], (int)ptBinsArr[ipt], (int)ptBinsArr[ipt+1], (int)(10*rapBinsArr[irap]), (int)(10*rapBinsArr[irap+1]), version.Data(), MupT.Data()), "READ");
...
//}}}
	...
	if(Weight) c1S->SaveAs(Form("Yield_distribution_Data_1S_%s_Acc%o_Eff%o_TnP%o_SigSys%o_BkgSys%o_MupT%s.pdf", version.Data(), isAccRW, isEffRW, isTnP, SigSys, BkgSys, MupT.Data()));
	else c1S->SaveAs(Form("Yield_distribution_Data_1S_%s_noWeight_MupT%s.pdf", version.Data(), MupT.Data()));
	...
	if(Weight) c1SN->SaveAs(Form("Yield_distribution_Data_1SN_%s_Acc%o_Eff%o_TnP%o_SigSys%o_BkgSys%o_MupT%s.pdf", version.Data(), isAccRW, isEffRW, isTnP, SigSys, BkgSys, MupT.Data()));
	else c1SN->SaveAs(Form("Yield_distribution_Data_1SN_%s_noWeight_MupT%s.pdf", version.Data(), MupT.Data()));

	c2S->cd();
	h2S->Draw("petext0");
	if(Weight) c2S->SaveAs(Form("Yield_distribution_Data_2S_%s_Acc%o_Eff%o_TnP%o_SigSys%o_BkgSys%o_MupT%s.pdf", version.Data(), isAccRW, isEffRW, isTnP, SigSys, BkgSys, MupT.Data()));
	else c2S->SaveAs(Form("Yield_distribution_Data_2S_%s_noWeight_MupT%s.pdf", version.Data(), MupT.Data()));

	c2SN->cd();
	h2SN->Draw();
	if(Weight) c2SN->SaveAs(Form("Yield_distribution_Data_2SN_%s_Acc%o_Eff%o_TnP%o_SigSys%o_BkgSys%o_MupT%s.pdf", version.Data(), isAccRW, isEffRW, isTnP, SigSys, BkgSys, MupT.Data()));
	else c2SN->SaveAs(Form("Yield_distribution_Data_2SN_%s_noWeight_MupT%s.pdf", version.Data(), MupT.Data()));

	c3S->cd();
	h3S->Draw("petext0");
	if(Weight) c3S->SaveAs(Form("Yield_distribution_Data_3S_%s_Acc%o_Eff%o_TnP%o_SigSys%o_BkgSys%o_MupT%s.pdf", version.Data(), isAccRW, isEffRW, isTnP, SigSys, BkgSys, MupT.Data()));
	else c3S->SaveAs(Form("Yield_distribution_Data_3S_%s_noWeight_MupT%s.pdf", version.Data(), MupT.Data()));

	c3SN->cd();
	h3SN->Draw();
	if(Weight) c3SN->SaveAs(Form("Yield_distribution_Data_3SN_%s_Acc%o_Eff%o_TnP%o_SigSys%o_BkgSys%o_MupT%s.pdf", version.Data(), isAccRW, isEffRW, isTnP, SigSys, BkgSys, MupT.Data()));
	else c3SN->SaveAs(Form("Yield_distribution_Data_3SN_%s_noWeight_MupT%s.pdf", version.Data(), MupT.Data()));
	...
	if(Weight) call->SaveAs(Form("Yield_distribution_Data_all_%s_Acc%o_Eff%o_TnP%o_SigSys%o_BkgSys%o_MupT%s.pdf", version.Data(), isAccRW, isEffRW, isTnP, SigSys, BkgSys, MupT.Data()));
	else call->SaveAs(Form("Yield_distribution_Data_all_%s_noWeight_MupT%s.pdf", version.Data(), MupT.Data()));
	...
	lt1N->DrawLatex(0.54,0.52,Form("%d < mult. < %d", (int)multBinsArr[0], (int)multBinsArr[mult_narr-1]));
	lt1N->DrawLatex(0.54,0.45,"|y^{#Upsilon}| < 2.4");
	if(Weight) callN->SaveAs(Form("Yield_distribution_Data_allN_%s_Acc%o_Eff%o_TnP%o_SigSys%o_BkgSys%o_MupT%s.pdf", version.Data(), isAccRW, isEffRW, isTnP, SigSys, BkgSys, MupT.Data()));
	else callN->SaveAs(Form("Yield_distribution_Data_allN_%s_noWeight_MupT%s.pdf", version.Data(), MupT.Data()));
}
