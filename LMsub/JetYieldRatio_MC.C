...
//}}}
	...
	for(Int_t imult = 0; imult < 2; imult++)
	{
		...
		{
			fjet[imult][ipt] = new TFile(Form("File/Sideband_%s_Mult_%d-%d_pt_%d-%d_rap_%d-%d_Trkpt_%d-%d_%s_MupT%s_%s.root", RorG.Data(), multMins[imult], multMaxs[imult], ptbins[ipt], ptbins[ipt+1], (int)(10*rapMin), (int)(10*rapMax), (int)TrkptMin, (int)TrkptMax, versions[imult].Data(), MupT.Data(), Fine.Data()));

			TH1D* htmpshort = (TH1D*) fjet[imult][ipt]->Get(Form("hJYield_short"));
			TH1D* htmplong = (TH1D*) fjet[imult][ipt]->Get(Form("hJYield_long"));
			if(ipt == 0 && htmplong->GetBinContent(1) > 0) htmpshort->Add(htmplong, -1);
...
cout << versions[imult] << ": short-long: " << ": Err: " << JCompE[imult][ipt] << endl;
			...
			JYield[imult][ipt] = JComp[imult][ipt];
			JYieldE[imult][ipt] = JCompE[imult][ipt];
...
cout << versions[imult] << ": signal: " << ": Err: " << JYieldE[imult][ipt] << endl;
	...
	c1->SaveAs(Form("Plots/JRatio/JRatio_%s_Mult_%d-%d_by_%d-%d_rap_%d-%d_Trkpt_%d-%d_%s_by_%s_%s_MupT%s.pdf", RorG.Data(), multMinhi, multMaxhi, multMinlow, multMaxlow, (int)(10*rapMin), (int)(10*rapMax), (int)TrkptMin, (int)TrkptMax, versionhi.Data(), versionlow.Data(), Fine.Data(), MupT.Data()));

	TFile* fout = new TFile(Form("File/JRatio_%s_Mult_%d-%d_by_%d-%d_rap_%d-%d_Trkpt_%d-%d_%s_by_%s_%s_MupT%s.root", RorG.Data(), multMinhi, multMaxhi, multMinlow, multMaxlow, (int)(10*rapMin), (int)(10*rapMax), (int)TrkptMin, (int)TrkptMax, versionhi.Data(), versionlow.Data(), Fine.Data(), MupT.Data()), "RECREATE");
	fout->cd();
	gJRatio->Write();
	fit1->Write();
	fout->Close();
