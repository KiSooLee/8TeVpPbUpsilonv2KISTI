...
//}}}
	...
	for(Int_t imult = 0; imult < 2; imult++)
	{
		...
		{
			fjet[imult][ipt] = new TFile(Form("File/trk_Sideband_%s_Mult_%d-%d_pt_%d-%d_rap_%d-%d_Trkpt_%d-%d_%s_MupT%s_%s.root", RorG.Data(), multMins[imult], multMaxs[imult], ptbins[ipt], ptbins[ipt+1], (int)(10*rapMin), (int)(10*rapMax), (int)TrkptMin, (int)TrkptMax, versions[imult].Data(), MupT.Data(), Fine.Data()));
			...
			//if ( (!isGen && imult == 0) || (imult == 1) ) JComp[imult][ipt] = htmpshort->GetBinContent(1) - htmplong->GetBinContent(1);
			//else JComp[imult][ipt] = htmpshort->GetBinContent(1);
			JComp[imult][ipt] = htmpshort->GetBinContent(1) - htmplong->GetBinContent(1);//RECO
			//JComp[imult][ipt] = htmpshort->GetBinContent(1);//GEN
			//if ( (!isGen && imult == 0) || (imult == 1) ) JCompE[imult][ipt] = TMath::Sqrt(htmpshort->GetBinError(1)*htmpshort->GetBinError(1)+htmplong->GetBinError(1)*htmplong->GetBinError(1));
			//else JCompE[imult][ipt] = htmpshort->GetBinError(1);
			JCompE[imult][ipt] = TMath::Sqrt(htmpshort->GetBinError(1)*htmpshort->GetBinError(1)+htmplong->GetBinError(1)*htmplong->GetBinError(1));//RECO
			//JCompE[imult][ipt] = htmpshort->GetBinError(1);//GEN
...
cout << versions[imult] << ": short-long: " << ": Err: " << JCompE[imult][ipt] << endl;
	...
	c1->SaveAs(Form("Plots/JRatio/trk_JRatio_%s_Mult_%d-%d_by_%d-%d_rap_%d-%d_Trkpt_%d-%d_%s_by_%s_MC_%s_MupT%s.pdf", RorG.Data(), multMinhi, multMaxhi, multMinlow, multMaxlow, (int)(10*rapMin), (int)(10*rapMax), (int)TrkptMin, (int)TrkptMax, versionhi.Data(), versionlow.Data(), Fine.Data(), MupT.Data()));

	TFile* fout = new TFile(Form("File/trk_JRatio_%s_Mult_%d-%d_by_%d-%d_rap_%d-%d_Trkpt_%d-%d_%s_by_%s_MC_%s_MupT%s.root", RorG.Data(), multMinhi, multMaxhi, multMinlow, multMaxlow, (int)(10*rapMin), (int)(10*rapMax), (int)TrkptMin, (int)TrkptMax, versionhi.Data(), versionlow.Data(), Fine.Data(), MupT.Data()), "RECREATE");
	fout->cd();
	gJRatio->Write();
	fit1->Write();
	fout->Close();
